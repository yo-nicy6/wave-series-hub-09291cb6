...
function thisy_world_info_meta_box($post) {
    ...
    $featured_order = get_post_meta($post->ID, '_wm_featured_order', true);
    ?>
    <div style="background:#fff3cd;padding:12px;border:1px solid #ffc107;border-radius:4px;margin-bottom:15px">
        ...
        <div style="margin-top:10px;padding-top:10px;border-top:1px dashed #ffc107">
            <label for="wm_featured_order" style="display:block;font-weight:600;color:#856404;margin-bottom:5px">
                <?php _e('Display Order (1 = First)', 'thisy-world'); ?>
            </label>
            <input type="number" id="wm_featured_order" name="wm_featured_order" value="<?php echo esc_attr($featured_order); ?>" min="1" max="12" style="width:60px;padding:5px;text-align:center" placeholder="1">
            <span class="description" style="display:block;margin-top:5px;font-size:11px"><?php _e('Lower numbers appear first. Items with same number sorted by date.', 'thisy-world'); ?></span>
        </div>
    </div>
...
function thisy_world_save_series_meta($post_id) {
    ...
    // Featured Order
    if (isset($_POST['wm_featured_order'])) {
        update_post_meta($post_id, '_wm_featured_order', intval($_POST['wm_featured_order']));
    }
}
...
function thisy_world_movie_info_meta_box($post) {
    ...
    <div style="background:#fff3cd;padding:12px;border:1px solid #ffc107;border-radius:4px;margin-bottom:15px">
        ...
        <div style="margin-top:10px;padding-top:10px;border-top:1px dashed #ffc107">
            <label for="wm_movie_featured_order" style="display:block;font-weight:600;color:#856404;margin-bottom:5px">
                <?php _e('Display Order (1 = First)', 'thisy-world'); ?>
            </label>
            <input type="number" id="wm_movie_featured_order" name="wm_movie_featured_order" value="<?php echo esc_attr($featured_order); ?>" min="1" max="12" style="width:60px;padding:5px;text-align:center" placeholder="1">
            <span class="description" style="display:block;margin-top:5px;font-size:11px"><?php _e('Lower numbers appear first. Items with same number sorted by date.', 'thisy-world'); ?></span>
        </div>
    </div>
...
function thisy_world_save_movie_meta($post_id) {
    ...
    // Featured Order
    if (isset($_POST['wm_movie_featured_order'])) {
        update_post_meta($post_id, '_wm_featured_order', intval($_POST['wm_movie_featured_order']));
    }
}
